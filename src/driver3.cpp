...
int main(int argc, char *argv[])
{
    ...
    for (int q=0; q<1; q++) { // each row of batches in the image. Change back to q=0, q<7
        ...
        for (int r=7; r<8; r++) { // change back to r=0, r<15
            if ((((r-7)*(r-7))+(q*q)) >= 50) {
        ...
        std::cout << "BATCH " << q << "  ---  RUNTIME: " << durationBatch.count() << " mins\n";
    }
